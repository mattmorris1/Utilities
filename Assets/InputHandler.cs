...
public static class InputHandler
{
    ...
    public static float convertToFloat(string inS)
    {
        float converted = 0F;
        try
        {
            converted = float.Parse(inS);
            return converted;
        }
        catch (FormatException e)
        {
    ...
    public static float getAverage(string inS)
    {
        float average = 0F;
        float count = 0F;
        try
        {
            string[] numbers = inS.Split(' ');
            foreach (var number in numbers)
            {
                average = average + float.Parse(number);
                count++;
            }
